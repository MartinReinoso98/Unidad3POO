...
class Hotel:
    ...
    def mostrarNumeroYPiso(self, tipo, nombre):
        ...
        for habitacion in self.__habitaciones:
            if habitacion.getTipo() == tipo:
                print(f"Hotel: {nombre} - Habitacion: {habitacion.getNumero()} - Piso: {habitacion.getPiso()}")
                encontrado = True
        return encontrado
    
    def mostrarCantPorPiso(self):
    ...
    def listar(self):
        print(f"{'Numero':<15}{'Piso':<10}{'Precio por noche':<20}{'Disponibilidad':<15}")
