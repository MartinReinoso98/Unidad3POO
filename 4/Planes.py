...
class Planes(ABC):
    ...
    def __str__(self):
        return f"Plan {self.__nombreC}: Duración {self.__duracion} meses, Cobertura {self.__cobertura}, Precio base {self.__precioBase}"
    ...
    def mostrarPlanes(self):
        print(f"Nombre de la compañía: {self.getNombreComp()}")
        print(f"Duración del plan: {self.getDuracion()} meses")
        print(f"Cobertura geográfica: {self.getCobertura()}")
